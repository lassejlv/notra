...
let can_go_back = pagination.previous_page.is_some();
let can_go_forward = pagination.next_page.is_some();
let page_info = format!(
    "Page {} of {}",
    pagination.current_page,
    pagination.total_pages.unwrap_or(1)
);
let item_count = format!(
    "{} total items",
    pagination.total_posts.unwrap_or(0)
);
```
</CodeGroup>
